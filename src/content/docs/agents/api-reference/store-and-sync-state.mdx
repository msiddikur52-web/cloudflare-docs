...
Every Agent has built-in state management capabilities with automatic persistence and real-time synchronization across all connected clients.
...
State synchronizes automatically with connected clients. Refer to the [Client SDK documentation](/agents/getting-started/) for full details.
...
These are durable operations and persist even if the workflow retries.
...
* Review the [API documentation](/agents/api-reference/agents-api/) for the Agents class.
